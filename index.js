...
app.post("/sort", (req, res) => {
    ...
    switch (waytoSort) {
        case "id": sort = "id";
            order = "DESC";
            break;
        case "updated_time": sort = "updated_time";
            order = "DESC";
            break;
        case "name": sort = "name";
            order = "ASC";
...
app.get("/edit/:id", async (req, res) => {
    ...
    try {
        ...
        res.render(path.join(__dirname, "views/new.ejs"), { book: bookToEdit, error: errorMsg });    
    } catch (error) {
        console.log(error);
    } 
}) 
...
app.post("/add", async (req, res) => {
    ...
    if (name && author && note && isbn && rating) {
        try {
            await db.query("INSERT INTO book_list (name, author, isbn, rating, note, updated_time) VALUES ($1, $2, $3, $4, $5, $6)", [name, author, isbn, rating, note, updatedTime]);
        } catch (error){
            console.log(error);
        } finally {
            errorMsg = "";
...
app.post("/update", async (req, res) => {
    ...
    if (name && author && note && isbn && rating) {
        try {
            await db.query("UPDATE book_list SET name = $1, author = $2, note = $3, isbn = $4, rating = $5, updated_time = $6 WHERE id = $7", [name, author, note, isbn, rating, updatedTime, id]);
        } catch (error){
            console.log(error);
        } finally {
            errorMsg = "";
